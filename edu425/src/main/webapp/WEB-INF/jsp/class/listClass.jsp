	...
	<table width="98%"  align="center" border=1>
		...
		<c:forEach items="${info.list}" var="clazz" varStatus="state">
			<tr class='${clazz.state.stateId==102?"endClass":"having"}'>
				...
				<td>${clazz.class_guideName}</td>
				<td>${clazz.job_guideName}</td>
				<td>${clazz.CreatedDate}</td>
				<td>${clazz.EndDate }</td>
				<td style="color:${clazz.STATEID==102?'red':'#000000'}">${clazz.STATEID}</td>
				<td>${clazz.COMMENT}</td>
		...
		<tr>
			<td style="text-align: left" colspan="11"><input type="button" value="增加新班级" onclick="window.open('../classAdd','_self')"></td>
		</tr>
